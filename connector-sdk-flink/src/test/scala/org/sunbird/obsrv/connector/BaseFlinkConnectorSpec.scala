...
abstract class BaseFlinkConnectorSpec extends FlatSpec with BeforeAndAfterAll {
  ...
  private def setupConnectorFramework(postgresConnect: PostgresConnect): Unit = {
    ...
    postgresConnect.execute("CREATE TABLE IF NOT EXISTS connector_instances ( id TEXT PRIMARY KEY, dataset_id TEXT NOT NULL REFERENCES datasets (id), connector_id TEXT NOT NULL REFERENCES connector_registry (id), data_format TEXT, connector_type TEXT NOT NULL, connector_config text NOT NULL, operations_config json NOT NULL, status TEXT NOT NULL, connector_state json, connector_stats json, created_by text NOT NULL, updated_by text NOT NULL, created_date TIMESTAMP NOT NULL DEFAULT now(), updated_date TIMESTAMP NOT NULL, published_date TIMESTAMP NOT NULL DEFAULT now());")
  }
  ...
  def loadConnectorData(postgresConnect: PostgresConnect): Unit = {
    ...
    postgresConnect.execute("insert into connector_instances(id, dataset_id, connector_id, data_format, connector_type, connector_config, operations_config, status, connector_state, connector_stats, created_by, updated_by, created_date, updated_date, published_date) " +
      "values ('c1', 'd1', '" + connConfig.getString("metadata.id") + "', 'json', '" + connConfig.getString("metadata.type") + "', '"+ connectorConfig +"','{}', 'Live', '{}', '{}', 'System', 'System', now(), now(), now());")
  }
